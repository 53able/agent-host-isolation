...
以前のnamed volume/root経路はApple Container 1.2.2で制限が効きませんでした。`nproc=32`のroot guestは40個のprocessを起動でき、16 MiB指定のvolumeには40 MiBを書き込めました。新しいtmpfs/非root経路では同じhostでprocessとdiskの拒否probeが通過しました。ただし残る7種類の敵対的検証とCPU、open-file、VM数の上限を同じ経路で検証するまで、自動振り分けは**blocked**です。手動の昇格requestは利用できます。
