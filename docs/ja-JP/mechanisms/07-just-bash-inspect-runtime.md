...
`task.strict_memory`は必須で、標準`inspect`では`false`に固定します。OS強制のメモリ上限が必要なTaskは、要求値がRSS監視の閾値以下でもJustBashに通しません。信頼できるhost controllerがcommandの開始前または実行中に要件を発見したら、privateなruntime handleで`blockInspectForStrictMemory({ runtime })`を呼びます。Adapterはattemptを停止し、`missing_capability: "strict-memory"`を持つ`InspectBlocked`を返します。
...
Host controllerはruntime handleをprivateに保ち、agent textや呼出し側の`missingCapability`ではなく、信頼できるTask policyでstrict-memory要件を判定します。`blockInspectForStrictMemory`を呼べるのはattemptがreadyまたはexecutingの間だけです。Adapterはattemptをblockedにし、実行中のJustBash commandをabortして、その終了を待ってからeventを返します。進行中の`executeInspectCommand`も同じblocked eventを返し、成功結果を返しません。以後の実行や重複した停止要求は拒否します。部分outputはresult gateへ取り込みません。Abort後もcommandが終了しない場合はhost worker watchdogが停止し、終了未確認のstrict-memory eventは発行しません。attemptがterminalになった後に要件が判明した場合は、新しいTask attemptが必要です。
