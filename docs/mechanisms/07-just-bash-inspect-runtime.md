...
The host-side `scripts/inspect_fetch_broker.py` is a narrower read-only snapshot path. It accepts a network-derived grant contract for a host fetch, pins a public DNS answer to the TLS connection, rechecks exact origin, canonical path, method, attempt, purpose, expiry, and remaining response-body budget on each redirect, and refuses caller headers. DNS resolution runs in a killable child process with an independent deadline. A denial, timeout, or cancellation revokes the broker instance. Every decision is written to a durable SQLite `FetchAuditStore`; production callers must choose a persistent, host-controlled database path. Its returned bytes and SHA-256 record are **pending result-gate review**; a caller must build a new standard, networkless manifest and content-checked input snapshot before JustBash sees them. The broker does not make direct `network-derived` JustBash execution available. The [Issue #5 evidence](../../evidence/inspect-fetch-issue5-20260923.md) records 26 passing probes and is `verified-for-tested-configuration` for the captured host, runtime, manifests, snapshots, and committed inputs. A failed or dirty rerun returns to `unverified`.

The only supported bridge is one-shot: a valid broker record is consumed by `scripts/inspect_snapshot_gate.py`, which verifies the source grant, audit record, body hash and target snapshot before importing bytes. `scripts/just_bash_runtime.mjs:createInspectRuntimeFromFetch` then constructs a new standard networkless runtime from that target snapshot. Replay, tampering, cross-task reuse, and a direct `network-derived` JustBash manifest are rejected. This bridge does not relax the standard validator or turn the derived profile on.
