...
For `strict-memory`, the new Apple Container `guest-build` manifest must set `task.strict_memory: true`; the generated request remains `automatic: false`. Automatic dispatch is disabled until memory exhaustion, cleanup, watchdog, artifact/result gate, and the full agent path are verified on the target host/runtime. A manual probe result for one limit does not authorize automatic dispatch.
