...
The host-side `scripts/just_bash_runtime.mjs` adapter converts an atomic JustBash command-not-found exit (127) into `InspectBlocked`. A compound command that exits 127 after earlier side effects remains a failed command, not an escalation signal. The adapter accepts a JustBash instance, never a host executor. It never falls back to a host shell or widens the current attempt. If native execution is required, create a new `guest-build` manifest with the **same Task ID**, a new manifest hash, and a new Task attempt for the Apple Container runtime; validate both the new manifest and its result gate before execution. Escalation is a request, not automatic authorization.
