...
class InspectFetchBroker:
    ...
    def revoke(self, reason: str = "cancelled") -> None:
        with self._lock:
            self._revoked = True
            self.audit.append({"decision": "revoked", "reason": reason})
    ...
    def _consume(self, grant: dict[str, Any], amount: int) -> None:
        key = grant["audit_record"]
        with self._lock:
            if self._revoked:
                raise FetchDenied("grant revoked")
            used = self._used.get(key, 0)
            if amount > grant["max_bytes"] - used:
                raise FetchDenied("grant byte budget exceeded")
    ...
    def fetch(self, url: str, *, method: str, scope: str, purpose: str, headers: dict[str, str] | None = None) -> tuple[bytes, dict[str, Any]]:
        ...
        try:
            ...
            for hop in range(6):
                ...
                try:
                    ...
                    if response.status in _REDIRECTS:
                        ...
                        self.audit.append({"decision": "redirect", "hop": hop, "url": current, "audit_record": grant["audit_record"]})
                        continue
                    ...
                    self.audit.append({"decision": "allowed", **record})
                    return body, record
                finally:
                    connection.close()
            ...
            self.audit.append({
                "decision": "denied", "reason": reason, "url": current,
                "task_id": self._manifest["task"]["id"],
                "attempt_id": self._manifest["task"]["attempt_id"],
                "manifest_hash": self._manifest_hash,
                "audit_record": grant["audit_record"] if grant else None,
            })
            self.revoke(reason)
