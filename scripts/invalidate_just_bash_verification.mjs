...
export function invalidateVerification(reason = "adversarial run started but has not completed") {
  const invalidatedAt = new Date().toISOString();
  mkdirSync(dirname(MANIFEST_PATH), { recursive: true });
  mkdirSync(dirname(EVIDENCE_JSON), { recursive: true });
  writeFileSync(EVIDENCE_JSON, `${JSON.stringify({ status: "unverified", captured_at: invalidatedAt, reason }, null, 2)}\n`);
