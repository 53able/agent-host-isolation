...
- [ ] Confirm the logical skill ID in frontmatter and workspace identity is `agent-host-isolation`; do not depend on the installation directory name.
- [ ] Keep `SKILL.md` under 500 lines.
- [ ] Use only relative paths with forward slashes in `SKILL.md`.
- [ ] Make the task manifest pass `scripts/validate-manifest.py`.
- [ ] Compile runtime operations with `scripts/apple_container_compiler.py` and execute only the returned argv array.
